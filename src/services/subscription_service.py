...
class SubscriptionService:
    ...
    ЕДИНАЯ БД с API (общая PostgreSQL). Премиум/триал и лимиты читаются
    напрямую из app_user_subscriptions — того же источника правды, что и API.
    """
    ...
    def check_expired_subscriptions(self) -> List[int]:
        ...
        try:
            expired = session.query(UserSubscription, User.telegram_id).join(
            ...
            for subscription, telegram_id in expired:
